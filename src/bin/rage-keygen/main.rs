...
#[cfg(feature = "yubikey")]
fn yubikey_setup() -> Result<Option<(String, String)>, age::Error> {
    use age::cli_common::read_passphrase;
    ...
    loop {
        ...
        if let Some(key) = keys.iter().find(|key| key.slot() == slot) {
            match key.certificate().subject_pki() {
                PublicKeyInfo::EcP256(pubkey) => {
                        ...
                        .interact()?
                    {
                        ...
                        break Ok(Some((stub.to_str(), recipient.to_str())));
                    }
                }
                ...
                .interact()?
            {
                ...
                let mut serial = [0; 20];
                getrandom(&mut serial).expect("Should not fail");
